...
from apitonio3 import bottle, jinja2_env

def serve_template(templatename, **kwargs):
    jinja2_env.get_template(templatename)
...
def get_oauth(token):
    ...
    try:
        ...
        return template("400.tpl", status_code = str(408), error_msg = "Answer is not JSON")
    if r.status_code == 200:
        ...
        if not resp.has_key('error'):
            ...
            if resp["error"] == "invalid_grant":
                return serve_template("400.tpl", status_code = str(404), error_msg = resp["error_description"])
                
            else:
                
                return serve_template("400.tpl", status_code = str(404), error_msg = resp["error_description"])
    else:
        return serve_template("400.tpl", status_code = str(r.status_code), error_msg="Unable to authenticate throught assests.cortext.net. Please retry and then contact your admin and ask him gently")
...
def lock_file(hashkey, token):
    ...
    if r.status_code == "200":
        ...
        if not resp.has_key('lock_info'):
            return serve_template("400.tpl", status_code = r.status_code, error_msg = "unable to requestslock: assets.cortext.net is unreachable")
...
def unlock_file(callback_json,hashkey,token, updated_at):
    ...
    if r.status_code == 200:
        try:
            ...
            return serve_template("400.tpl", status_code = str(408), error_msg = "Answer is not JSON")
    else:
        return serve_template("400.tpl", status_code = r.status_code, error_msg = "unable to unlockfile: assets.cortext.net is unreachable")
...
def get_data(hashkey="f7ef64c1ded0dc2f831ec34089ffc2d2", access_token=None, db="test"):
    ...
    if not os.access(config["filename"], os.R_OK):
        ...
        try:
            ...
            return serve_template("400.tpl", status_code = str(404), error_msg = "Connexion error: unable to get data on the doc: assets.cortext.net is unreachable")
...
def get_metadata(hashkey="f7ef64c1ded0dc2f831ec34089ffc2d2", access_token = None):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            return template("400.tpl", status_code = str(r.status_code), error_msg = "Unable to get metadata on the doc: assets.cortext.net is unreachable")
            return template("400.tpl", data = {"status_code":r.status_code, "error_msg":"unable to get metadata on the doc: assets.cortext.net is unreachable"})
    except requests.ConnectionError:
        return serve_template("400.tpl", status_code=str(r.status_code), error_msg="Connexion error: unable to get metadata on the doc: assets.cortext.net is unreachable")
